...
namespace Microsoft.Research.Naiad.Examples.Netflix
{
  ...
  public class LongSumReducer : ReducerBase
  {
    public long value;

    public LongSumReducer(long value)
    {
      this.value = value;
    }

    public override ReducerBase Add(ReducerBase other)
    {
      LongSumReducer otherCast = (LongSumReducer)other;
      return new LongSumReducer(value + otherCast.value);
    }
  ...
  public class Netflix : Example {
    ...
    public void Execute(string[] args) {
      using (var computation = NewComputation.FromArgs(ref args)) {
        ...
        if (args.Length != 4) {
          Console.WriteLine("Please specify input files");
        }
        ...
        // var testRes = ratingJoin.GenericAggregator(row => new LongPair(row.first, row.third),
        //                                            row => new LongSumReducer(row.second));
        // var intResult = testRes.Select(row => new Pair<LongPair, long>(row.v1, ((LongSumReducer)row.v2).value));

        var matmultfinalPrj = intResult.Join(ratingSel,
                       intRes => intRes.v1.s, // movId
                       ratSel => ratSel.first, // movId
                       (intRes, ratSel) =>
                       new LongTriple(intRes.v1.s,
                                      intRes.v2 * ratSel.third,
                                      ratSel.second));
        ...
        var prediction = predicted.Max(row => row.v1.t,
                                       row => row.v2)
